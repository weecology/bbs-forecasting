...
add_ranefs = function(df, last_training_year) {
  ...
  # Run a linear mixed effects model on richness, using only the training portion
  richness = df %>%
    filter(year <= last_training_year) %>%
    group_by(site_id, year, observer_id) %>%
    summarize(richness = n()) %>%
    ungroup

  lmer_model = lmer(richness ~ (1|site_id) + (1|observer_id), data = richness)
