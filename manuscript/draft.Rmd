...
hybrids. Due to limitations of the environmental data (below), we did not
include any data from outside the continental United States. Observed richness
from 1982 (the first complete year of NDVI data) to 2003 were used for training
the models, and data from 2004 to 2013 was used for testing their performance.
Only BBS routes that were sampled during 70% of the years in the training period
were used (i.e., routes with at least 16 annual observations). All data from
routes that did not meet BBS acceptable criteria were excluded, as were routes
that had not been visited during at least 70% of the years in our model-training
window. The resulting dataset included [[N]] species and [[N]] sites. Among
sites,  richness varied from [[N]] to [[N]] with an average richness of [[N]]
species. Within-site richness variation (among years) was small (standard
deviation of about 5 species, after controlling for observer effects).
...
in a notable clustering of predicted values near ~60 species and the poorest
...
or the ARIMA model [[Not sure what the bit about within-site variability means
here. Also, didn't Naive usually win when observer effects weren't corrected
for?]] (Figure 3). In general, each model's error increased more-or-less
linearly with time, although the slopes and initial error rates varied across
models (Figure 3). The site-level models (i.e. the baselines and the ARIMA
methods) tended to do very well (Figure 3) on short time horizons, especially in
terms of RMSE.
...
stack was the most overconfident, with observed richness values frequently
falling well outside their confidence limits (Figure 3C). As noted elsewhere
[@harris2015], this overconfidence is a product of the assumption in stacked
SDMs that errors in the species-level predictions are independent. The JSDM and
macroecological model has the best coverage, though each model still
underestimated the possible variation in richness. The "naive" model
was the only model whose confidence intervals were too wide (Figure 3), which
can be attributed to the rapid rate at which these intervals expand as the time
lag increases (Figure 1).
...
other hand, the GBM-based "macroecological" model and especially the mistnet
JSDM had reasonably well-calibrated uncertainty estimates, even in the presence
of uncorrected observer biases (Figure 3C). This prevented their deviance from
rising as quickly as the site-level models as the observer population evolved
and resulted in better comparative performance for these models in terms of
deviance (Figure 3B).
...
otherwise accommodate. Most forecasts exhibited only small changes in RMSE.
However these changes were sufficient to change the rank ordering of model
performance. For example, the "naive" model generally had the lowest average
error when observer effects were not explicitly accounted for, switching to the
"average" and ARIMA model once observer differences were corrected. Improvements
in coverage were notable for all models except Naive and JSDM, meaning that
...
latent variables were already accounting for observer-related uncertainty. These
differences meant that, in terms of deviance, neither the JSDM nor the
macroecological model model benefitted as much from the as the site-level models
did from the observer corrections. As a result, the average and ARIMA models
maintained a small average advantage over the JSDM in terms of deviance at
longer time scales, rather than falling behind after about five years.
...
in the forecast relative to a simple baseline [[weather citation]]. We recommend
...
assessment impossible. Currently assessments at these long time-scales can be
only be made with a small number of opportune datasets. For examples hindcasts
have been used to asses models of species richness [@alger2009, distler2015] and
...
evaluating trends in the error shorter term forecasts as done here it will be
