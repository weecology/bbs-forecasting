...
Predictions from the "average" model are centered on the average richness
observed during training, and the confidence intervals are narrow and
constant-width.

The second baseline, called the "naive" model [@hyndman2014], was a simple
autoregressive process with a single year of history: 

$$y_t = y_{t-1} + \epsilon_t$$

In contrast to the "average" model, whose predictions were based on the average
...
hindcasts the value observed in `r timeframe$last_train_year`), and the
confidence intervals expand rapidly as the predictions extend farther into the
future. Both models' richness predictions are centered on a constant value, so
neither model can anticipate any trends in richness or any responses to future
