...
well (Figure 3), especially on short time horizons, and especially in terms of
RMSE. Of these, the "Naive" model performed especially well at sites with
uncorrected observer turnover (since its predictions were best-calibrated to
observers that were active in later years).

Most models produced confidence intervals that were too narrow, indicating
overconfident predictions. The random forest-based SDM stack was the most
overconfident model, with observed richness values frequently falling well
outside its confidence limits (Figure 3C). This stacked SDM's narrow
predictive distribution also caused it to have notably higher deviance (Figure
3B) than the next-worst model, even though its point estimates were not
unusually bad in terms of RMSE (3A), and the other models' relative performance
was not nearly as sensitive to the choice of error metric. As discussed
elsewhere [@harris2015], this overconfidence is a product of the assumption in
stacked SDMs that errors in the species-level predictions are independent.
Because they avoided this assumption, the GBM-based "macroecological" model and
...
calibration improved both models' deviance relative to the baseline models,
especially as the observer population evolved (Figure 3B). The "naive" model was
...
resulted in improvements to nearly all metrics of fit (Figure 4). The
improvements in point estimates (as measured by RMSE) primarily resulted from
large improvements in a small number of forecasts where a large difference
between observers resulted in a large shift in predicted richness values that
the models could not otherwise accommodate. The "Average" and ARIMA models
generally showed the largest improvement here, and the Naive model showed the
smallest (since it was already very responsive to observer turnover). As a
result, the "Average" and "ARIMA" models tended to perform the best, once
observer differences were accounted for. Improvements in coverage were notable
for all models except Naive and JSDM, meaning that unmodeled observer effects
had been partly responsible for the poor quantification of uncertainty. As a
result, the average and ARIMA models maintained a small average advantage over
the JSDM in terms of deviance at longer time scales, rather than falling behind
after about five years as in Figure 3B.
