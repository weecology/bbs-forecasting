...
simple baselines. Since site-level richness is relatively stable in so many
systems [@dornelas2014], spatial validation will not be enough: a model can have
high accuracy across spatial gradients without being able to predict changes
over time. This gap between spatial and temporal accuracy is known to be
...
SDMs' poor temporal predictions are particularly sobering, as these models have
been one of the main foundations for estimates of the predicted loss of
biodiversity to climate change over the past decade or so [@thomas2004;
@thuiller2011; @urban2015]. One could argue that these conclusions are unfair to
SDMs and related methods, which are generally used to predict large directional
changes over multi-decade timescales rather than fluctuations within a single
decade. It is certainly possible that these models' relative performance would
improve over time or with larger shifts in richness, and future research should
explore this possibility. Nevertheless, our results have important implications
for ecologists' long-term predictions regarding large changes in species
richness, for two reasons. First, after controlling for observer effects, we
found no evidence that these models have any ability to track changes in
richness over time at all (Figure 6). Given that prediction error steadily
increases over time (Figure 5), we expect that long-term SDM-based forecasts
will generally be at least as poor as decade-scale ones. 

Our results also highlight the importance of comparing multiple modeling
...
recommend following the example of @ye2015 and adopting this approach in future
...
mostly enabled them to avoid this problem. Macroecological models appear to share 
this advantage, while being considerably easier to implement. 
