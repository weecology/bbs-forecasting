...
a Gaussian error distribution (rather than a count distribution) for the reasons
discussed below .
...
horizon. Finally, we decomposed each model's squared error into two components:
one described the squared error associated with site-level means, while the
other described the squared error associated with annual fluctuations in
richness within a site. This decomposition tells us the extent to which each
...
over time. Both components of the mean squared error were lower for the time
series models than for the environmental models.](../figures/barcharts.png)
...
The most commonly used method to forecast future biodiversity, SDMs
