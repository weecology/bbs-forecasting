...
where observer turnover caused a large shift in the reported richness
values. The naive baseline was less sensitive to these shifts, because it
largely ignored the richness values reported by observers that had retired by
the end of the training period (Figure 1). The average model, which gave equal
weight to observations from the whole training period, showed a larger decline
in performance when not accounting for observer effects -- especially in terms
of coverage. The performance of the mistnet JSDM was notable here, because its
...
prediction's accuracy by more than 10 species. As a result the inclusion of
observer effects results in average improvements in overall model
fit.](../figures/observers.png)
...
given ecological quantity. This includes comparing process-based [e.g.
@kearney2009] and data-driven models, as well as comparing the accuracy of
forecasts to simple baselines to determine if the modeled forecasts are more
accurate than the naive assumption that the world is static [@ye2015].
...
primarily focus on using spatial models and space-for-time substitutions
