...
![Performance of the six models, trained with data from 1982-2003, when 
predicting one year into the future (2004) or ten years into the future (2013).
In general, the single-site models (**A.**) outperformed the environmental
models (**B.**). The accuracy of the predictions generally declined as the
timescale of the forecast was extended from 2004 to 2013.](../figures/scatter.png)
...
![Site/year difference between the average model and all other models in absolute
error (**A.**) and deviance (**B.**). All testing years (2004-2013) are used here. 
None of the five other models provided a consistent improvement over the "average"
baseline, indicated by a horizontal line. Models' absolute error was generally
similar or larger than "average" model, with large outliers in both directions.
The other models' deviance was also generally higher than the "average"
baseline.](../figures/model_violins.png)
...
than the other models (since its predictions are too confident, see Fig. 1), and 
that the "naive" model's deviance grows relatively quickly (as its predictions
become broader and less informative, see Fig. 1). **C.** Coverage of a model's
95% confidence intervals indicates how often the observed values fall inside the
predicted range. For well-calibrated models, the coverage of the 95% confidence
intervals should be 95% (indicated by the black line). As in the previous panel,
the "naive" model's predictive distribution is too wide (capturing almost all of
the data) and the stacked SDM's predictive distribution is too narrow (missing
almost a third of the observed richness values by 2014).](../figures/performance_time.png)
...
richness is relatively stable, model evaluations that show high
...
biodiversity to climate change [@thomas2004; @thuiller2011; @urban2015].
...
studies compared the SDM and direct richness modeling approaches, and reported
...
uncertainty [@harris2015; @warton2015]. In fact, JSDMs and direct richness modeling
...
aggregated across similar time scales. More evaluation
