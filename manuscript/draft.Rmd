...
forecasts for breeding bird species richness at `r numbers$N_predict_sites`
sites throughout the continental United States, following ten suggested best
practices for forecasting. Forecasts were made for up to a decade after the
final training observations. While all of the models we evaluated produce
reasonable forecasts, none of them consistently outperforms the simple baseline
of constant long-term average richness. At present, our best forecast for future
richness is thus to assume that it is unchanging, and current modeling
approaches do not reliably improve on this simple baseline. Most of the best
practices implemented in this study directly influence the outcome of the
forecasts and result in different conclusions from those made in previous
studies. This emphasizes the importance of following good practice in making
ecological forecasts and of starting to actively make more forecasts in such a
way that they can be quickly evaluated and improved on.
...
can predict the future. These components of good forecasting and evaluation
(Box 1) broadly involve: 1) expanding the use of data to include biological and
...
focus on using correlative models to predict richness changes over time from
...
estimator; this number averaged [[N]] for the non-observer data and [[N]] for
the observer-corrected data.
...
difficulty fitting the data. Since richness had a relatively large mean ([[N]])
and all models already produce continuous richness estimates (due to summing
probabilities in the (J)SDM case), we defined model performance for all models
in terms of continuous Gaussian errors instead of using discrete count
distributions. When a model was run multiple times for the purpose of correcting
for observer effects, we used the mean of those runs' point estimates as our
final point estimate and we calculated the uncertainty using the law of total
variance (i.e.  the average of the model runs' variance, plus the variance in
the point estimates).

We evaluated the performance of each model for forecasting using the data for
each year between `r timeframe$last_train_year` and `r timeframe$end_yr`. We used
...
the poorest performance ($R^2$ `r numbers$env_R2s`, versus XX-XX for the
within-site methods).
...
While the models generally did well in absolute terms (Figure 4), none tended to
outperform the "average" baseline from the generalized linear mixed model
(Figure 4A). [[Stats go here]].  The auto-ARIMA was generally the
best-performing non-baseline model, but in many cases (67% of the time), the
best-fitting ARIMA model chosen by the model selection procedure had no
autoregressive, drift, or moving average terms, making it similar to the
"Average" model.

![](../figures/model_violins.png)
...
the GBM-based "macroecological" model and especially the mistnet JSDM had
reasonably well-calibrated uncertainty estimates (Figure 4B); as a result, their
relative performance was higher in terms of deviance than in terms of RMSE. The
"naive" model was the only model whose confidence intervals were too wide
(Figure 4B), which can be attributed to the rapid rate at which these intervals
expand as the time lag increases (Figure 1).
...
predicted richness values by 10+ species. These changes usually improve the
model's predictions, but not always.](../figures/observers.png)
...
over time. This issue has also recently been highlighted as an issue
