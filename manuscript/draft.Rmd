...
**Time series models**
* These models only looked at one site at a time and looked for patterns in 
  univariate time series.
* Auto-ARIMA (Hyndman 2016). Choose a differencing order, then do forward stepwise
  model selection based on AICc.
    * Autoregressive: current state depends on previous state(s)
    * Moving average: current observation error depends on previous observation error
    * Integrated/differencing: is analysis performed on raw data, on sequential 
      differences of the raw data, on sequential differences of sequential differences,
      etc.
    * default settings, except seasonal = FALSE
* Auto-ARIMA + site-level environmental variables over time.
...
* We chose the `gbm` package for fitting boosted regression trees as our example of
  a nonlinear "macroecological model".
    * Works by fitting thousands of small models sequentially, and then adding up
      their predictions
    * the individual small models were regression trees with [[settings]].
...
* To make it feasible to fit this large number of models, we used a custom wrapper 
  ...
  by combining many independent models ("trees") fit to different versions of the training
