...
**We agree that accounting for spatial autocorrelation in the observer model would have improved our estimates of the site-level random effects. However, we do not believe that this change would affect the paper's main results. Improvements would primarily benefit the "average" baseline, which is based directly on the observer model. As a result, it would be even harder for the three environmental models to outperform the baseline---especially since (as you noted), spatial and environmental explanations for variation in species richness trade off with one another.**
